...
public class Menu extends JMenuBar{
	...
	private void userMenu(){
		...
		JMenuItem Usun = new JMenuItem("Usuñ");
		JMenuItem Dodaj = new JMenuItem("Dodaj nowy");
		...
		menuSamochody.add(Usun);
		menuSamochody.add(Dodaj);
		menuUmow.add(Umow);
		...
		oProgramie.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
					...
					tf.setText("Projekt z Technologii Obiektowych PK");
		...
		Usun.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(Klient.ID !=0){
					int id = Panel.pobierzID();
					if(id != -1){
						Klient.Command("Delete from car where car_id = '" +  id +"'");
					}else{
						JFrame jf = new InfoBox("Wybierz, który samochód chcesz usun¹æ");
					}
					}else{
						JFrame f = new InfoBox("Zaloguj siê aby korzystaæ z funkcji programu");
					}
